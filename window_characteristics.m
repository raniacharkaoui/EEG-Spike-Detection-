function window_characteristics(current,file)
% This function creates a window 
% in which the user can change the characteristics (Time in/out lists,
% channels) of the selected file
% Author : Kevin Nzeyimana (kevin.nzeyimana@reseau.eseo.fr)

name = ['Choose characteristics for ' file(current).Name];
...
% Creation of the object Uicontrol Start 
     ...
     'units','normalized',...
     'FontWeight','bold',...
     'position',[0.35 0.05 0.25 0.05],...
     'string','Save',...    
     'callback',@save_recordings,...
     'tag','bouton_start');
 ...
 % Update handles structure
    ...
    % *************************************
    %  By default, all channels are not   *
    %     chosen for second change        *
    % *************************************
...
function save_recordings(~,~)
    timeIn = getappdata(gcbf,'Algo_timeIn');
    timeOut = getappdata(gcbf,'Algo_timeOut');
    timeDebrec = getappdata(gcbf,'Algo_timeDebrec');
    nrElectrodeLeft = getappdata(gcbf,'nrElectrodeLeft');
    nrElectrodeRight = getappdata(gcbf,'nrElectrodeRight');
    file = getappdata(gcbf,'file');
    current = getappdata(gcbf,'current');
    isAllChecked = getappdata(gcbf, 'isAllchecked');
    
    [Recordings] = AttribuePat(file(current).Recordings.path,file(current).Name,timeIn,timeOut,timeDebrec,nrElectrodeLeft,nrElectrodeRight,current,isAllChecked);
